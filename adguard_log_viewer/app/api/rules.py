...
async def create_rule(payload: RuleWrite, db: DbDep, state: StateDep) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(status_code=409, detail="A rule with that name already exists") from err
    state.reload_engine()
...
async def test_rule(payload: dict[str, Any], state: StateDep) -> dict[str, Any]:
    ...
    if isinstance(draft, dict):
        ...
        if rule is None:
            raise HTTPException(
                status_code=400,
                detail="The rule needs a name, at least one condition and at least one tag",
            )
    ...
    if state.engine is None:
        raise HTTPException(status_code=503, detail="The categorisation engine is not ready")
    return {
        "domain": domain,
