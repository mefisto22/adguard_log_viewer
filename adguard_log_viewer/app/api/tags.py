...
async def update_tag(tag_id: int, payload: TagUpdate, db: DbDep) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail="A tag with that name already exists") from err
    if not changed:
        raise HTTPException(status_code=404, detail="Tag not found or nothing to change")
    return ok()
