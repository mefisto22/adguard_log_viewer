...
A DNS query log viewer and analyzer for AdGuard Home, as a Home Assistant app.

The app speaks English and Hungarian and follows Home Assistant's language,
...
* The **official AdGuard Home app**, installed and running.
...
Under Home Assistant OS an app **cannot read another app's data
directory**, so `querylog.json` is out of reach. This app therefore uses
AdGuard Home's **HTTP API**, which is only reachable once the AdGuard app's
web port is published. (The reasoning in full: `ARCHITECTURE.md`.)

1. Home Assistant → **Settings → Apps → AdGuard Home**.
2. The **Configuration** tab → the **Network** section.
3. Enter a free host port next to **Web interface** (`80/tcp`).
4. **Save**, then **restart** the AdGuard Home app.
...
> value: this app uses the port you chose, because it asks the Supervisor for
...
the AdGuard app works, and it survives an update.

### Or give the address directly

If you would rather not publish the port, or AdGuard does not run under Home
Assistant, put the address in this app's **AdGuard Home URL** option. **This
...
By default the AdGuard app protects that port with **Home Assistant login**.
So:

| The AdGuard app's `leave_front_door_open` | What to give this app |
|---|---|
...
3. Install the **AdGuard Log Viewer** app.
...
On its first start the app reads AdGuard's log backwards as far as the
...
|---|---|---|
| `adguard_url` | *(empty)* | The address you open AdGuard at. Left empty it is asked of the Supervisor — which needs the AdGuard app to publish `80/tcp`. If it does not, put the address here; a bare port (`9000`) is enough. |
| `adguard_username` | *(empty)* | A Home Assistant username (see section 2). |
| `adguard_password` | *(empty)* | Its password. Home Assistant masks it and the app never logs it. |
| `adguard_verify_ssl` | `false` | Turn this on only if AdGuard is reachable over HTTPS with a valid certificate. |
| `adguard_slug` | *(empty)* | Only if auto-discovery would find the wrong app (e.g. `a0d7b954_adguard`). |
...
| `log_level` | `info` | At `debug` the app may log queried domains — for troubleshooting only. |

What the **Settings** page changes without a restart: language, theme, density,
default period, page size, retention, ingest on/off. The app options appear
there read-only.
...
If the app store shows the repository but it is empty, or the app is
...
the top, and search for the app's name. A rejected configuration looks like
this:
...
1. **Reload the repository.** Install app → ⋮ → *Check for updates*. The
   Supervisor caches repositories, so a fix does not show up straight after a
   push. From the command line: `ha addons reload`.
2. **Architecture.** This app runs on `aarch64` and `amd64` machines. If your
...
The **Settings** page says what the app can see. The usual causes:

| Symptom | Cause | Fix |
|---|---|---|
| `The AdGuard Home address could not be worked out` | The AdGuard app publishes no host port for `80/tcp` | Section 2: set a port, **or** give `adguard_url`. The Settings page's **How it was looked up** row shows step by step what was found, and which ports the Supervisor reports for the AdGuard app |
...
| `No AdGuard Home app was found` | Discovery found no AdGuard | Set `adguard_url` by hand |

### No new records arrive

* Look at the *Last poll* row on the **Settings** page.
* Check that the query log is enabled in AdGuard Home — the app warns
...
* There are only two outbound destinations: the Supervisor (`http://supervisor`)
  and the AdGuard app's local address.
* The category lists live in the repository, offline — nothing is downloaded at
  runtime.
* Below `debug` level the app does not write queried domains to its log.
...
* All data lives in the app's `/data` directory
...
DNS query log böngésző és elemző az AdGuard Home-hoz, Home Assistant alkalmazásként.

Az alkalmazás magyarul és angolul tud, és a Home Assistant nyelvét követi — ezen a
...
* A **hivatalos AdGuard Home alkalmazás** telepítve és futva.
...
Home Assistant OS alatt egy alkalmazás **nem tudja olvasni egy másik alkalmazás
adatkönyvtárát**, tehát a `querylog.json` fájlt közvetlenül nem érjük el. Ezért
ez az alkalmazás az AdGuard Home **HTTP API-ját** használja, amit viszont csak akkor
lehet elérni, ha az AdGuard alkalmazás webes portja publikálva van. (A miértek
részletesen: `ARCHITECTURE.md`.)

1. Home Assistant → **Beállítások → Alkalmazások → AdGuard Home**.
2. **Konfiguráció** fül → **Hálózat** szekció.
3. A **Web interface** (`80/tcp`) sorba írj be egy szabad hoszt portot.
4. **Mentés**, majd az AdGuard Home alkalmazás **újraindítása**.
...
> ez az alkalmazás azt a portot használja, amit te megadtál, mert a Supervisortól
...
alkalmazás működését, és frissítés után is megmarad.

### Vagy add meg a címet közvetlenül

Ha nem akarod publikálni a portot, vagy az AdGuard nem a Home Assistant alatt
fut, írd be a címet ennek az alkalmazásnak az **AdGuard Home URL** opciójába. **Ez
...
Az AdGuard alkalmazás ezt a portot alapértelmezésben **Home Assistant
bejelentkezéssel** védi. Ezért:

| AdGuard alkalmazás `leave_front_door_open` | Mit adj meg ennek az alkalmazásnak |
|---|---|
...
1. **Beállítások → Alkalmazások → Alkalmazás telepítése → ⋮ → Tárolók**.
2. Add hozzá ennek a repónak az URL-jét.
3. Telepítsd az **AdGuard Log Viewer** alkalmazást.
...
Első indításkor az alkalmazás visszamenőleg is beolvassa az AdGuard logját, a
...
|---|---|---|
| `adguard_url` | *(üres)* | Az a cím, amin az AdGuardot megnyitod. Üresen hagyva a Supervisortól kérdezi le — ehhez az AdGuard alkalmazásnak publikálnia kell a `80/tcp` portot. Ha nem publikálja, írd ide a címet; elég egy önmagában álló port is (pl. `9000`). |
| `adguard_username` | *(üres)* | Home Assistant felhasználónév (lásd 2. pont). |
| `adguard_password` | *(üres)* | A hozzá tartozó jelszó. A HA maszkolja, az alkalmazás soha nem logolja. |
| `adguard_verify_ssl` | `false` | Csak akkor kapcsold be, ha az AdGuard HTTPS-en, érvényes tanúsítvánnyal érhető el. |
| `adguard_slug` | *(üres)* | Csak ha az automatikus felderítés rossz alkalmazást találna (pl. `a0d7b954_adguard`). |
...
| `log_level` | `info` | `debug` szinten az alkalmazás a lekérdezett domaineket is logolhatja — csak hibakereséshez. |
...
ki/be. Az alkalmazásbeállítások ott csak olvashatóan jelennek meg.
...
### Az alkalmazás meg sem jelenik a tároló hozzáadása után

Ha a telepíthető alkalmazások között megjelenik a tároló, de üres, vagy az alkalmazás hiányzik,
...
**Supervisor** naplót, és keress rá az alkalmazás nevére. Egy elutasított
...
1. **Frissítsd a tárolót.** Alkalmazás telepítése → ⋮ → *Frissítés*. A Supervisor
   gyorsítótárazza a tárolókat, ezért egy javítás push után nem jelenik meg
   azonnal. Parancssorból: `ha addons reload`.
2. **Architektúra.** Ez az alkalmazás `aarch64` és `amd64` gépeken fut. Ha a
...
A **Settings** oldal megmondja, mit lát az alkalmazás. A gyakori okok:

| Tünet | Ok | Megoldás |
|---|---|---|
| `Az AdGuard Home címe nem volt megállapítható` | Az AdGuard alkalmazás nem publikál hoszt portot a `80/tcp`-hez | 2. pont: állíts be portot, **vagy** add meg az `adguard_url`-t. A Settings oldal **Hogyan lett megkeresve** sora lépésről lépésre megmutatja, mit talált, és hogy a Supervisor milyen portokat jelent az AdGuard alkalmazásra |
| `Az AdGuard Home nem érhető el itt: <cím>` | A cím megvan, de nem válaszol | Ellenőrizd, hogy az AdGuard fut-e, és hogy tényleg ezen a porton érhető el |
| `HTTP 401` vagy `403` | Hiányzó vagy rossz belépési adat | Adj meg érvényes **Home Assistant** felhasználót és jelszót |
| `JSON helyett ezt adta vissza: text/html` | A kérést az nginx bejelentkeztető rétege fogta el | Ugyanaz: a felhasználónév/jelszó nem stimmel |
| `A Supervisoron keresztül nem található AdGuard Home alkalmazás` | A felderítés nem talált AdGuardot | Add meg kézzel az `adguard_url`-t |
...
* Ellenőrizd, hogy az AdGuard Home-ban a query log be van-e kapcsolva — ha nincs,
  az alkalmazás külön figyelmeztet.
...
* Hálózati kapcsolat csak két cél felé megy: a Supervisor (`http://supervisor`)
  és az AdGuard alkalmazás lokális címe.
* A kategória-listák a repóban vannak, offline — futásidőben semmit nem tölt le.
* Az alkalmazás a lekérdezett domaineket `debug` szint alatt nem írja ki a logba.
...
* Minden adat az alkalmazás `/data` könyvtárában van
